...
/**
 * @file SLM_scenario1Context.hpp
 * @author S. Martorana (salvatoremartorana@hotmail.com)
 * @author F. Abrignani (federignoli@hotmail.it)
 * @brief Alias SLM_scenario1Context.
 * @version 0.1
 * @date 2023-10-06
 * @private
...
namespace SLM_scenario1{
    ...
    void threadFunction(thread_context* c ,int i, boost::fairness::slim_priority_mutex<5>& m) {
        c->lock(&m,i);
        ret.push_back(i);
        m.unlock();
    }

    void controlThread(thread_context* c, boost::fairness::slim_priority_mutex<5>& m) {
        c->lock(&m);
        c->switchContext();
        m.unlock();
    }
