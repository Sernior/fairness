...
namespace SPM_scenario8{
    ...
    void threadFunction(thread_context* c, int i, boost::fairness::shared_priority_mutex<7>& m, std::mutex& sm) {
        if (i<3) {
            c->lock(&m, i);
            ret.push_back(i);
            m.unlock();
    ...
    static constexpr auto executeSchedulingSequence = []{
        ...
        auto thread_0 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_1 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_2 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_3 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_4 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_5 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_6 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
        auto thread_7 = std::tuple{&threadFunction, distribution(gen), std::ref(m), std::ref(sm)};
